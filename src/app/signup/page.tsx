...
<h1 className="text-center text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-purple-500 to-blue-500 mb-2">Sign Up</h1>
...
<div className="w-24 h-24 rounded-full overflow-hidden border-2 border-purple-500 shadow-lg bg-gray-700">
                                {preview ? (
...
<label htmlFor="image" className="absolute bottom-0 right-0 bg-purple-500 hover:bg-purple-600 text-white p-2 rounded-full cursor-pointer shadow-lg transition">
                                <FaCamera size={12} />
                            </label>
                        </div>
<input id="image" name="image" type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
                        <p className="text-xs text-gray-400">Upload your profile picture</p>
                    </div>
...
<button type="button" onClick={() => setIsShowPassword(!isShowPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 z-10 text-gray-400">
...
<Button className='flex-1 bg-gradient-to-r from-purple-500 to-blue-500 text-white font-bold h-11 rounded-xl shadow-lg' type="submit" isDisabled={loading}>
